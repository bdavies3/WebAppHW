...
def record_view(city_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM playerstats WHERE id=%s', city_id)
    result = cursor.fetchall()
...
@app.route('/edit/<int:>int:player_id', methods=['POST'])
def form_update_post(person_id):
    ...
    sql_update_query = """UPDATE playerStats t SET t.fldName = %s, t.fldAge = %s, t.fldHeight_inches = %s, t.fldTeam= %s, t.fldPosition= %s t.fldWeight_lbs
    %s WHERE t.id = %s """
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO playerStats (fldName,fldAge,fldHeight,fldWeight) VALUES (%s, %s,%s, %s) """
    cursor.execute(sql_insert_query, inputData)
    mysql.get_db().commit()
    return redirect("/", code=302)


@app.route('/delete/<int:int:player_id>', methods=['DELETE'])
def form_delete_post(person_id):
    cursor = mysql.get_db().cursor()
    sql_delete_query = """DELETE FROM playerStats WHERE id = %s """
...
@app.route('/api/v1/players/<int:int:player_id>', methods=['GET'])
def api_retrieve(person_id) -> str:
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM playerStats WHERE id=%s', person_id)
    result = cursor.fetchall()
    json_result = json.dumps(result);
    resp = Response(json_result, status=200, mimetype='application/json')
    return resp
...
@app.route('/api/v1/players/<int:int:player_id>', methods=['PUT'])
def api_edit(person_id) -> str:
    cursor = mysql.get_db().cursor()
    content = request.json
    inputData = (content['fldName'], content['fldAge'], content['fldHeight'],
                 content['fldWeight'], person_id)
    sql_update_query = """UPDATE playerStats t SET t.fldName = %s, t.fldAge = %s, t.fldHeight = %s, t.fldWeight = 
        %s WHERE t.id = %s """
    cursor.execute(sql_update_query, inputData)
    mysql.get_db().commit()
...
@app.route('/api/players/<int:int:player_id>', methods=['DELETE'])
